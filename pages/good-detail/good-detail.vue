<template>
	<view style="height: 100%">
		<view class="top_contain">
			...
			<swiper indicator-color="#e9e9e9" indicator-active-color="#ffc300" circular indicator-dots autoplay
				interval="3000" :duration="800">
				<swiper-item v-for="(pic, index) in goodinfo.pics" :key="index" @tap="previewImage(goodinfo.pics, index)">
					<view class="top_img">
			...
			<view class="price_tags_box">
				...
				<!-- 成色和品质标签 -->
				<view class="tags">
					<uni-tag :text="goodinfo.condition" type="success"></uni-tag>
					<uni-tag :text="goodinfo.category" type="warning"></uni-tag>
				</view>
		...
		<view class="center_contain">
			<uni-section class="mb-10" title="发布信息" titleFontSize="32rpx" titleFontWeight="600" titleColor="#000"
				type="line" />
		</view>
		<!-- 发布信息 -->
		<view v-if="first_title">
			...
			<view class="address_box" v-if="goodinfo.delivery === '自提'">
				<view class="deliver_title">取货地点：</view>
				<view class="deliver_place">{{ goodinfo.place }}</view>
			</view>
			<view class="palceInput_box" v-if="goodinfo.delivery == '配送'">
				<input @input="placeInput" placeholder="卖家可以帮送，请填写您的收货地址" />
			</view>
			<view class="notes_box" v-if="goodinfo.notes !== ''">
				<view class="notes">
		...
		<view class="tabbar">
			...
			<view class="t_card">
				...
				<text>聊天</text>
				<button class="t_button" @click="go" data-go="/pages/kefu/kefu"></button>
			</view>
			<view class="buy_box">
				<view class="buy shadow" @tap="buy">{{ goodinfo.status == 0 ? "立即预定" : "刚刚被抢光了" }}</view>
			</view>
		</view>
	</view>
</template>
