<template>
	<view id="page">
		<VanTabs :activePage="activePage" animated @click="onClickVanTabs" class="van-tabs" color="#ffc300"
			background="#e9e9e9">
			<VanTab title="商品管理" class="good-container" color="#ffc300">
				<!-- 导航栏菜单 -->
				<view class="navbar">
					<view v-for="(item, index) in goodMenuItems" :key="index" class="navbar-item"
				...
				<view class="content">
					<uni-swipe-action>
						<view class="swipe-aciton-wrap" style="" v-for="(item, index) in list" :key="index">
							<uni-swipe-action-item :right-options="rightOptions" data-item="item"
								@click="onClickSwipeItem($event, item)">
								<div class="wrap" @click="toDetailPage(item, 'good')">
									<div class="left">
									...
									<div class="center">
										<div class="top">
											...
											<div class="price">
												{{ item.price }}<text
													style="font-size: 28rpx; font-weight: normal; color: #01a3ff">{{ item.status == 1 ? "（待确认）" : "" }}</text>
											</div>
										</div>
										<div class="bottom">
											<div class="label">
			...
			<VanTab title="书籍管理" class="good-container" color="#ffc300">
				<!-- 导航栏菜单 -->
				<view class="navbar">
					<view v-for="(item, index) in bookMenuItems" :key="index" class="navbar-item"
						:class="{ active: activeIndex2 === index }" @click="changeActive2(index)">
						{{ item.name }}
						<view class="item-line" :class="{ active: activeIndex2 === index }"></view>
					</view>
				</view>
				<!-- 提示文本 -->
				<div class="tip">对着商品卡片 <span class="tip-key">左划</span> 可进行成交、下架和重新编辑操作哦~</div>
				...
				<view class="content">
					<uni-swipe-action>
						<view class="swipe-aciton-wrap" style="" v-for="(item, index) in list" :key="index">
							<uni-swipe-action-item :right-options="rightOptions" data-item="item"
								@click="onClickSwipeItem($event, item)">
								<div class="wrap" @click="toDetailPage(item, 'book')">
									<div class="left">
										<image class="img-good" :src="item.bookinfo.pic" mode="aspectFill" />
									</div>
									<div class="center">
										<div class="top">
											<div class="desc">
										...
										<div class="bottom">
											<div class="label">
												...
												<div class="address" v-if="item.deliveryid === 0">
													<image class="icon-address" src="../../static/label/address.svg" mode="" />
													<span class="text-address">{{ item.place }}</span>
												</div>
												<div class="quality">
	...
	export default {
		...
		methods: {
			...
			toDetailPage(item, type) {
				...
				if (type === "book") {
					uni.navigateTo({
						url: "/pages/book-detail/book-detail?scene=" + item._id
					});
				}
			},
			setOption(newVal) {
...
<style lang="scss" scoped>
	...
	.wrap {
		...
		// box-shadow: 2rpx 3rpx 10rpx 2rpx rgb(201, 199, 199);
