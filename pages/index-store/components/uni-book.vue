<template>
	<view class="uni-book">
		<view v-if="!iscard" class="list-layout">
			<view class="left_scroll" :class="{ fixed: localScrollTop >= 750 }">
			...
			<view v-if="localScrollTop >= 750" class="zhandi">
			</view>
			<view class="right_scroll">
				<view scroll-y v-if="list.length > 0" :scroll-top="localScrollTop" @scrolltolower="more"
...
<script>
	...
	export default {
		...
		methods: {
			// 监测内容的滚动
			onContentScroll(e) {
				this.localScrollTop = e.detail.scrollTop;
			},
			//跳转搜索
			search() {
				uni.navigateTo({
			...
			detail(e) {
				let that = this;
				uni.navigateTo({
					url: "/pages/book-detail/book-detail?scene=" + e.currentTarget.dataset.id,
				});
			},
		},
	};
