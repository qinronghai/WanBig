<template>
	<view style="height: 100%">
      ...
      src="/static/images/startBg.jpg"></image> -->
		<image v-if="backgroundimage" class="bg" :src="backgroundimage"></image>

		<view class="chatroom">
			<view class="good-card-wrap" v-if="isShowThisGood">
				<view class="good-card">
					<div class="left">
						<image :src="goodInfo.show.pic" mode="scaleToFill" @click="toDetail" />
					</div>
					<div class="right">
						<div class="right-top">
							<div class="title">{{ goodInfo.show.title }}</div>
							<uni-icons type="closeempty" size="20" color="#666666" @click.stop="closeThisGood"></uni-icons>
						</div>
						<div class="right-bottom">
							<div class="price">￥{{ goodInfo.show.price }}</div>
							<button open-type="" hover-class="button-hover" @click="sendThisGood">
								发送商品
							</button>
						</div>
					</div>
			...
			<scroll-view class="body" scroll-y :scroll-with-animation="scrollWithAnimation" :scroll-top="scrollTop"
				:scroll-into-view="scrollToMessage" @scrolltoupper="onScrollToUpper">
				<view :id="'item-' + index" :class="'message ' + (openId == item._openid ? 'message__self' : '')"
					...
					<view class="main">
						...
						<block v-if="item.msgType === 'image'">
							<view class="image-wrapper">
								<view class="loading" v-if="item.writeStatus > -1 && item.writeStatus != null">{{ item.writeStatus }}%
								</view>
								<image :src="item.tempFilePath || item.imgFileID" :data-fileid="item.tempFilePath || item.imgFileID"
									class="image-content" :style="item.imgStyle" mode="scallToFill" @tap="onMessageImageTap">
								</image>
							</view>
						</block>
                ...
                <view
                  ...
                  {{ item.filename }}
						...
						<block v-if="item.msgType === 'card'">
							<van-card :thumb-link="thumblink + item.goodInfo._id" :price="item.goodInfo.show.price"
								:desc="item.goodInfo.show.author || ''" :title="item.goodInfo.show.title"
			...
			<view class="footer">
				<view class="message-sender" v-if="userInfo">
					<image src="/static/images/chat/photo.png" class="btn-send-image" mode="scaleToFill" @tap="onChooseImage">
					</image>

					<input class="text-input" type="text" confirm-type="send" @input="textInputOnChange"
						@confirm="onConfirmSendText" cursor-spacing="20" :value="textInputValue" />
				</view>
			</view>
		</view>
	</view>
		...
		methods: {
				...
				if (this.from !== "chat-list") {
					this.$nextTick(async () => {
						...
						let {
							...
							price,
							pic
						} = this.from === "good-detail" ? this.goodInfo.show : this.goodInfo;
						const showCard = {
							title,
							author,
							price,
			...
			async initRoom(from) {
				this.tryFun(async () => {
					...
					const {
						data: initList
					} = await db.collection(collection).where(this.mergeCommonCriteria()).orderBy("sendTimeTS", "desc")
						.get();
			...
			async onRealtimeMessageSnapshot(snapshot) {
				...
				if (snapshot.type === "init") {
					this.setData({
						chats: [...this.chats, ...[...snapshot.docs].sort((x, y) => x.sendTimeTS - y
						.sendTimeTS)], //将新消息追加到聊天记录中，确保按照时间顺序
					});
