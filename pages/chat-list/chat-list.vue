<template>
	<view>
		<uni-search-bar v-model="searchValue" placeholder="搜索好友" @input="searchFriend" />
		<uni-section v-if="isSearch" class="mb-10" title="搜索结果" titleFontSize="30rpx" type="line"></uni-section>

		<uni-list :border="true" v-for="(item, index) in getList" :key="index">
			<uni-swipe-action>
				<uni-swipe-action-item :right-options="options" @click="onClick">
					<uni-list-chat :title="item.userInfo.info.nickName" @click="peoplepage(item, 1)" :clickable="true"
						:avatar="item.userInfo.info.avatarUrl" :note="item.note" :time="item.time"
						:badge-text="item.badgeText"></uni-list-chat>
				</uni-swipe-action-item>
			</uni-swipe-action>
		</uni-list>
		<view v-if="getList === 0" class="no-search-result">
			暂无好友
		</view>
		<view v-if="searchResults.length === 0 && isSearch" class="no-search-result">查无此人...</view>
	</view>
</template>
		...
		methods: {
			...
			searchFriend(e) {
				...
				this.debounce(() => {
					...
					// 筛选符合关键词的名字
					const searchResults = this.friendList.filter((item) => item.userInfo.info.nickName.includes(
					friendNameKey));
					...
					success: async (res2) => {
						...
						const promises = friends.map(async (item, index) => {
							try {
								...
								const {
									data
								} = await db.collection("user").doc(item.userInfo._id).get();
								const friendInfo = data;
								console.log("data,index :>> ", data, index);
								// 比对头像和昵称是否有变化
								if (friendInfo.info.avatarUrl !== item.userInfo.info.avatarUrl || friendInfo.info
									.nickName !== item.userInfo.info.nickName) {
								...
								const {
									...
									time,
									badgeText
								} = await that.getUnreadedMsg(item.userInfo._openid, item.userInfo.info.nickName);
			...
			async getUnreadedMsg(openid, name) {
				...
				if (unreadedList.length > 0) {
					return {
						note: unreadedList[unreadedList.length - 1].textContent === "发送了商品卡片" ? "商品卡片" : unreadedList[unreadedList
					...
					return {
						note: readedList[readedList.length - 1].textContent === "发送了商品卡片" ? "商品卡片" : readedList[readedList.length -
							1].textContent,
