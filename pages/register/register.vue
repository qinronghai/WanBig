		...
		<view class="contain">
			<view class="card">
				...
				<picker mode="multiSelector" @change="bindMultiPickerChange" @columnchange="bindMultiPickerColumnChange"
					...
					<view class="campus">
						<block v-if="multiIndex[0] !== -1"> 当前选择：{{ newArr[0][multiIndex[0]] }}--{{ newArr[1][multiIndex[1]] }}
						</block>
				...
				<view class="contact">
					<input type="number" @input="phoneInput" :value="phonenum" maxlength="20" placeholder-class="pache"
			...
			<!-- 底部确认 -->
			<button class="confirm" open-type="getUserInfo" @getuserinfo="getUserInfo">
				注册并登录
			</button>
