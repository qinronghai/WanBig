	...
	<view style="height: 100%">
		...
		<van-transition :show="show_a" name="fade-right" custom-class="block" :duration="entime">
			<view class="a_contain">
				<view class="a_inp shadow">
					<image class="a_scan" src="/static/images/scan.png"></image>
					<input @confirm="confirm" @input="isbnInput" :value="isbn" type="number" maxlength="13"
						placeholder-class="a_pla" placeholder="请输入书本的ISBN号码" />
					<view class="a_confirm" @tap="confirm">确认</view>
				</view>
				<image class="a_indication" src="/static/images/up.svg" />
				<view class="a_des">1.您可以直接在上面输入框输入ISBN码</view>
				<image class="a_isbn" src="/static/images/isbn.jpg" />
				<view class="a_des">2.您还可以直接通过摄像头扫描出ISBN码</view>
				<image class="a_indication" src="/static/images/down.svg" />
			</view>
			<view class="a_sao">
				<button class="shadow" @tap="scan">
					点击扫码
				</button>
			</view>
		</van-transition>
			...
			<view class="b_ccontain">
				<view class="b_card">
					<view class="b_bar">
						...
						<view class="b_input">
							<uni-number-box :value="price" @change="priceChange" :min="1" :step="0.5" :max="100">
							</uni-number-box>
						</view>
					</view>
					...
					<view class="b_bar">
						...
						<radio-group class="kind-group" @change="kindChange">
							<label class="b_radio" v-for="(item, index) in kind" :key="index">
								<radio color="#ffc300" :value="item.id" :checked="item.check" />
					...
					<block v-if="chooseCollege">
						...
						<view class="b_bar">
							...
							<picker mode="multiSelector" @change="bindMultiPickerChange" @columnchange="bindMultiPickerColumnChange"
								...
								<view class="b_picker">
									<block v-if="multiIndex[0] !== -1"> {{ newArr[0][multiIndex[0]] }}--{{ newArr[1][multiIndex[1]] }}
									</block>
					...
					<block>
						...
						<view class="b_bar">
							...
							<picker :range="condition" range-key="name" :value="c_id" @change="choCondition">
								<view class="b_picker">
									<block v-if="c_id != -1">当前选择：{{ condition[c_id].name }}</block>
									<block v-if="c_id == -1">请选择成色</block>
									<image class="right" src="/static/images/right.png" />
								</view>
							</picker>
						</view>
					</block>
					...
					<view class="b_bar">
						...
						<radio-group class="kind-group" @change="delChange">
							<label class="b_radio" v-for="(item, index) in delivery" :key="index">
								<radio color="#ffc300" :value="item.id" :checked="item.check" />
				...
				<view class="b_publish" @tap="check_pub">马上发布</view>
			</view>
		</van-transition>
		<!-- 步骤三 -->
		<van-transition :show="show_c" name="fade-right" custom-class="block" :duration="entime">
			<view class="c_contain" :style="'height:' + (systeminfo.windowHeight - 66.8) + 'px'">
				<image class="c_success" src="/static/images/success.png"></image>
				<view class="c_title">恭喜您 发布成功</view>
				<view class="c_des">订单的实时状态将通过微信的服务通知告知您</view>
				<view class="c_see" @tap="detail">查看详情</view>
				<view class="c_again shadow" @tap="initial">再来一本</view>
			</view>
		</van-transition>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			afterRead(e) {
				...
				console.log(e, "实拍图");
				// { detail: { file } }
				const pics = Array.isArray(file) ? file.map((item) => ({
					url: item.thumb
				})) : [{
					url: file.thumb
				}];
			...
			async subscribNews() {
				...
				const id1 = "Ilo99ZTw6DoohkCTaLRMhTegzom5-Wf3tLDdgxAHLv4"; //预定通知-买家预定后通知卖家该商品被预定
				const id2 = "V76QqsleOiTKnU4pMfEtSWMC5c58WvQ2yANcEJJNe5c"; //订单状态通知-1.买家取消预定 2.买家取消交易 3.买家确认收货
				const id3 = "nYKQaIjCDZPc7MICBPsAU7SfsVhZZdRzJhGAn_x2234"; //未读消息通知-聊天新消息
